    ...
    if not playlist_description or not limit:
        ...
        if "error" in openai_call:
            ...
            if "error" in create_playlist_call:
                ...
                st.error(f"Create playlist error: {err['tool']}: {err['exception']}")
                st.text(err.get("traceback", ""))
                st.session_state.errors.append(err)
                st.stop()
            ...
            if not playlist_id:
                ...
                st.session_state.errors.append(
                    {"tool": "create_playlist", "exception": "no playlist id returned"}
                )
                st.stop()
            ...
            for s in song_objects:
                ...
                search_call = safe_call("Spotify (search_for_item)", search_for_item, q)
            ...
            st.session_state.playlist_info = {
                ...
                "songs": [{"title": s.title, "artist": s.artist} for s in song_objects],
...
if st.session_state.playlist_info:
    ...
    st.write(f"**Resolved tracks added (approx):** {info.get('resolved_track_count')}")
