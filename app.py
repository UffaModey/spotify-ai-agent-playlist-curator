        ...
        if not playlist_description or not limit:
            ...
            if "error" in openai_call:
                ...
                try:
                    ...
                    content = openai_call["result"]["choices"][0]["message"][
                        "content"
                    ].strip()
                    ...
                    if not song_objects:
                        st.warning(
                            "No valid songs were generated by OpenAI. Try again with a clearer description."
                        )
                    else:
                        ...
                        st.success(
                            f"✅ '{playlist_title}' generated! Review and select songs below."
                        )
                        ...
                        for idx, song in enumerate(song_objects):
                            ...
                            checked = st.checkbox(
                                label, value=select_all, key=f"chk_{idx}"
                            )
                            if checked:
                        ...
                        st.write(
                            f"**Selected {len(selected_songs)} out of {len(song_objects)} songs**"
                        )

                        if not st.button("Create Playlist with Selected Songs"):
                            # Stop execution here — wait until button is clicked
                            st.stop()
