...
if not st.session_state.token_info:
    ...
    if st.button("Generate Song Suggestions"):
        ...
        if not playlist_description or not limit:
            ...
            st.info("Generating song suggestions...")
            ...
            if "error" in openai_call:
                ...
                try:
                    ...
                    content = openai_call["result"]["choices"][0]["message"]["content"].strip()
                    parsed = json.loads(content)

                    playlist_title = parsed.get("playlist_title", f"AI: {playlist_description[:40]}")
                    songs_data = parsed.get("songs", [])

                    # Validate and convert song objects
                    song_objects = [
                    ...
                    if not song_objects:
                        st.warning("No valid songs were generated by OpenAI. Try again with a clearer description.")
                    else:
                        # Save to session for later use
                        st.session_state.generated_songs = song_objects
                        st.session_state.playlist_title = playlist_title

                        st.success(f"✅ '{playlist_title}' generated! Review and select songs below.")
                        st.subheader("🎧 Review Song Suggestions")

                        # Display songs with checkboxes
                        selected_songs = []
                        select_all = st.checkbox("Select All Songs", key="select_all")

                        for idx, song in enumerate(song_objects):
                            label = f"{song.title} — {song.artist}"
                            checked = st.checkbox(label, value=select_all, key=f"chk_{idx}")
                            if checked:
                                selected_songs.append(song)

                        st.session_state.selected_songs = selected_songs
                        st.write(f"**Selected {len(selected_songs)} out of {len(song_objects)} songs**")

                        if st.button("Create Playlist with Selected Songs"):
                            if not selected_songs:
                                st.warning("Please select at least one song.")
                            else:
                                st.info("Creating playlist on Spotify...")
